    ...
    \column{0.54\textwidth}
      \begin{block}{从 3D CT 到 2D axial slice}
        \begin{itemize}
          ...
          \item 根据 affine 信息确定 axial 轴，避免错误切片方向；
          \item 保留前景切片及其上下文，稀疏保留纯背景切片；
          \item 生成训练、验证、测试 CSV，保证 patient-level split。
        \end{itemize}
      \end{block}
    \column{0.42\textwidth}
      \begin{alertblock}{为什么强调 axial 轴}
        KiTS23 是 3D CT 分割任务。切片方向错误会让模型看到不符合解剖结构的图像，导致定性结果和 Dice 指标同时失真。
      \end{alertblock}
  ...
  \begin{columns}[T]
    ...
    \column{0.44\textwidth}
      \begin{exampleblock}{本项目生成式模型}
        \begin{itemize}
          \item conditional VAE：潜变量生成 mask；
          \item conditional diffusion：从 noisy mask 逐步去噪得到 clean mask；
          \item DDIM 采样：比较 1/5/10/25 步。
        \end{itemize}
      \end{exampleblock}
  \end{columns}
...
\begin{frame}{训练曲线}
  ...
  \end{columns}
  \begin{alertblock}{观察}
    cVAE 收敛较早但测试泛化较弱；x0 diffusion 训练到后期仍有提升，最终验证 Mean Dice 达到 0.532。
  \end{alertblock}
\end{frame}
...
\begin{frame}{主实验结果}
  \begin{block}{KiTS23 axial test set}
    ...
    \begin{tabular}{llrrrrr}
      \toprule
      模型 & 类型 & Mean & Kidney & Tumor & Cyst & ms/img \\
      \midrule
      ...
      x0 Diffusion, 5 steps & 生成式 & \metricup{0.434} & 0.820 & 0.449 & 0.032 & 67.43 \\
      \bottomrule
    \end{tabular}
  \end{block}
  \begin{columns}[T]
    \column{0.48\textwidth}
      \begin{exampleblock}{生成式模型结果}
        x0 diffusion 明显优于 cVAE，说明逐步去噪式 mask 生成比一次性潜变量解码更稳定。
      \end{exampleblock}
    \column{0.48\textwidth}
      \begin{alertblock}{主要差距}
        判别式模型仍然更准确、更快；cyst 是所有模型中最困难的类别。
      \end{alertblock}
  \end{columns}
\end{frame}
...
\begin{frame}{采样步数消融}
  \begin{columns}[T]
    ...
    \column{0.50\textwidth}
      \begin{alertblock}{结论}
        在当前设置下，采样步数并非越多越好。5 步采样取得最高 Dice，10/25 步没有继续提升，却显著增加推理时间。
      \end{alertblock}
      \begin{exampleblock}{解释}
        分割任务更关注语义区域是否正确，多步迭代可能在 tumor/cyst 小区域累积误差。
      \end{exampleblock}
  \end{columns}
\end{frame}
  ...
  \begin{columns}[T]
    ...
    \column{0.49\textwidth}
      \begin{alertblock}{核心结论}
        \begin{itemize}
          \item x0 diffusion 的 Mean Dice 为 0.434，明显优于 cVAE 的 0.201；
...
\begin{frame}{局限与后续方向}
  \begin{block}{主要局限}
    \begin{itemize}
      ...
      \item pixel-space mask diffusion 推理较慢，5 步已约 67 ms/image；
